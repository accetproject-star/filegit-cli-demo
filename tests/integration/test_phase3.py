...
def test_phase3_dashboard_and_traces():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            runner.invoke(typer_app, ["trace", "record", "--prompt", "err", "--action", "err", "--description", "err"], env={"FILEGIT_PRIVATE_KEY": ""})
            
            res = runner.invoke(typer_app, ["trace", "record", "--prompt", "Write a function", "--action", "file_edit", "--description", "Wrote function"])
            assert res.exit_code == 0
            
            # Record a second action to test append
            res = runner.invoke(typer_app, ["trace", "record", "--prompt", "Write a function", "--action", "test_run", "--description", "Passed"])
            assert res.exit_code == 0
            
            # Seal the trace
            res = runner.invoke(typer_app, ["trace", "seal", "--agent-id", "test-agent", "--bundle-id", bundle_id])
            if res.exit_code != 0:
                print("SEAL ERROR OUTPUT:", res.stdout)
            assert res.exit_code == 0
            
            # Test sealing a missing trace
            res = runner.invoke(typer_app, ["trace", "seal", "--agent-id", "nonexistent", "--bundle-id", bundle_id])
            assert res.exit_code != 0
