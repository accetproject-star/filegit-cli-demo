...
from filegit.domain.errors import (
    FileGitError,
    HashMismatchError,
    ManifestError,
    SignatureError,
)
from filegit.domain.models import (
    FileGitManifest,
    OwnerConfig,
    Policy,
    Signature,
...
from filegit.domain.ports import CryptoPort, FileSystemPort, HashingPort
...
class FileGitUseCases:
    ...
    def pack(
        self, repo_path: Path, private_key_path: Optional[Path] = None
    ) -> FileGitManifest:
        ...
        if not priv_key_str and private_key_path:
            ...
            if bool(st.st_mode & 0o077):
                raise FileGitError(
                    f"Private key file {private_key_path} has insecure "
                    f"permissions. Must be 0600."
                )
            priv_key_str = self.fs.read_text(private_key_path).strip()

        if not priv_key_str:
            raise FileGitError(
                "Private key not provided. Set FILEGIT_PRIVATE_KEY or use --key-path."
            )
        ...
        # Generate a new DID and public key for the manifest
        # (in a real app, this is configured)
        # For MVP, we derive public key from private key, but PyNaCl
        # signing keys already have it.
        import nacl.encoding
        import nacl.signing

        signing_key = nacl.signing.SigningKey(
            priv_key_bytes, encoder=nacl.encoding.RawEncoder
        )
        ...
        manifest = FileGitManifest(
            ...
            owner=OwnerConfig(
                ...
                verification_method=VerificationMethod(
                    public_key_multibase=pub_key_b58
                ),
            ),
            created_at=now,
            valid_from=now,
        ...
        manifest.signatures.append(
            Signature(key_id=f"{did}#keys-1", signature=signature_b58)
        )
    ...
    def verify(
        self, repo_path: Path, manifest_path: Path, require_trace: bool = False
    ) -> bool:
        ...
        is_valid = self.crypto.verify(
            pub_key_bytes, signable_bytes, sig_record.signature
        )
        if not is_valid:
            raise SignatureError(
                "Cryptographic signature verification failed. "
                "The manifest was tampered with."
            )
        ...
        for policy in manifest.policies:
            ...
            if local_hash != policy.hash:
                raise HashMismatchError(
                    f"Hash mismatch for {policy.path}. "
                    f"Expected {policy.hash}, got {local_hash}"
                )
            ...
            for trace_file in traces_dir.glob("trace-*.json"):
                try:
                    ...
                    if trace.bundle_id != manifest.id:
                        raise ManifestError(
                            f"Trace {trace.trace_id} does not match current bundle."
                        )
                    ...
                    if not self.crypto.verify(
                        pub_key_bytes,
                        trace.to_signable_bytes(),
                        trace.signature.signature,
                    ):
                        raise SignatureError(
                            f"Trace {trace.trace_id} has an invalid signature."
                        )
                except Exception as e:
                    raise ManifestError(
                        f"Invalid trace format in {trace_file.name}: {e}"
                    )
