...
class FileGitManifest(BaseModel):
    ...
    def to_signable_bytes(self) -> bytes:
        ...
        excluding the 'signatures' field, which is used for creating/verifying signatures.
        """
        data = self.model_dump(mode='json', exclude={'signatures'})
        # Use Pydantic's JSON serialization to ensure deterministic datetime formatting etc.
