...
def read_policy(policy_name: str) -> str:
    ...
    if not manifest_path.exists():
        raise ValueError("No FileGit manifest found in the current directory.")  # pragma: no cover
    ...
    raise ValueError(f"Policy '{policy_name}' not found in the authorized context bundle.")

@mcp.tool()
def verify_compliance() -> str:
    """Verify if the current repository state complies with the FileGit authorized context bundle."""
    uc = get_use_cases()
    repo_path = Path.cwd()
    manifest_path = repo_path / ".filegit" / "manifest.json"
    ...
    try:
        ...
        if is_valid:
            return "✅ VERIFIED: The context bundle is authentic and unmodified. You are in compliance."
    except Exception as e:  # pragma: no cover
