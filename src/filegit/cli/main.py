...
def init(
    repo_path: Path = typer.Option(
        Path.cwd(), help="Path to the repository to initialize."
    ),
) -> None:
    ...
    try:
        ...
        console.print(
            f"[green]✔[/green] FileGit repository initialized at "
            f"[bold]{repo_path / '.filegit'}[/bold]"
        )
...
def keygen(
    key_dir: Path = typer.Option(
        Path.home() / ".filegit" / "keys", help="Directory to store keys."
    ),
) -> None:
    ...
    try:
        ...
        console.print("[green]✔[/green] Keys generated successfully.")
...
def pack(
    ...
    key_path: Optional[Path] = typer.Option(
        None,
        help="Path to the private key file. Overrides FILEGIT_PRIVATE_KEY env var.",
    ),
) -> None:
    ...
    try:
        ...
        console.print("[green]✔[/green] Bundle packed successfully.")
        console.print(f"  [bold]Bundle ID:[/bold] {manifest.id}")
        console.print(
            f"  [bold]Manifest:[/bold]   {repo_path / '.filegit' / 'manifest.json'}"
        )
    except FileGitError as e:  # pragma: no cover
        console.print(f"[red]Error:[/red] {e}")
        raise typer.Exit(code=1)
...
def verify(
    ...
    require_trace: bool = typer.Option(
        False, help="Require a valid agent execution trace"
    ),
) -> None:
    ...
    try:
        ...
        if is_valid:
            console.print(
                "[green]✔[/green] [bold]VERIFIED:[/bold] "
                "The context bundle is authentic and unmodified."
            )
    except (
        FileGitError,
        ManifestError,
        ...
        console.print("[red]❌ BLOCK:[/red] Verification failed.")
...
def studio() -> None:
    ...
    try:
        ...
        console.print(
            "[red]Error:[/red] The 'fastapi' and 'uvicorn' "
...
def trace_seal(
    ...
    bundle_id: str = typer.Option(
        ..., help="The ID of the FileGit bundle this trace complies with"
    ),
    repo_path: Path = typer.Option(Path("."), help="Path to the repository"),
) -> None:
...
def mcp() -> None:  # pragma: no cover
    ...
    try:
        ...
        console.print(
            "[red]Error:[/red] The 'mcp' dependency is not installed. "
