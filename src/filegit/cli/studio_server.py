...
@app.post("/api/pack")
def pack_policies():
    """Pack and sign the policies into a manifest using the private key from the environment."""
    import os
    if "FILEGIT_PRIVATE_KEY" not in os.environ:
        raise HTTPException(status_code=400, detail="FILEGIT_PRIVATE_KEY environment variable is not set. Cannot sign the policies.")
